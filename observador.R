...
dir.create("observador")

news_observador = as.data.table(read_excel(file.path("data", "news_observador.xlsx")))
news_observador[, id := paste0("news", ...1)]

tmp = as.POSIXct(strptime(news_observador$date, "%Y-%m-%d %H:%M"), tz = "GMT")
news_observador[, posix := tmp]
news_observador[, date_new := as.Date(posix)]
news_observador[, date_old := date]
news_observador[, date := date_new]
news_observador[, date_new := NULL]
setkeyv(news_observador, "posix")
saveRDS(news_observador, file.path("data", "news_observador.rds"))
  ...
  removeWords(
    removeNumbers(
      removePunctuation(
        removePunctuation(
          removeWords(
            tolower(news_observador$text), stopwords("pt"))), ucp = TRUE)), stopwords("pt"))))

names(texts) = news_observador$id
...
texts = lapply(texts, function(x) x[!x %in% stopwords("pt")])
texts = lapply(texts, function(x){
  removeNumbers(x)
  x[nchar(x) > 1]
})
...
roll = RollingLDA(texts, dates,
                  ...
                  init = "2020-02-07",
                  K = k)
saveRDS(roll, file.path("roll", paste0(k, "_observador.rds")))
...
dir.create(file.path("observador", "topWordsPerChunk"))
for(i in 1:K){
  out = topwords_chunks[[i]]
  colnames(out) = as.character(roll$chunks$end.date)
  write.csv(out,
            file = file.path("observador", "topWordsPerChunk", paste0(i, "_", topwords[i], ".csv")),
            fileEncoding = "UTF-8")
}
...
for(rel in c(0.8, 0.85)){
  ...
  saveRDS(quantiles1, file.path("observador", paste0("quantiles", rel, ".rds")))
  saveRDS(sim1, file.path("observador", paste0("sim", rel, ".rds")))
  ...
  pdf(file.path("observador", paste0("wordimpact", rel, ".pdf")), height = 8, width = 10)
  for(k in seq_len(K)){
    zaehler = 0
  ...
  fwrite(events, file = file.path("observador", paste0("changes", rel, ".csv")))
  
  pdf(file.path("observador", paste0("changes", rel, ".pdf")), height = 8, width = 8)
