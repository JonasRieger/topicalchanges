...
dir.create(file.path("guardian", "topWordsPerChunk"))
for(i in 1:K){
  out = topwords_chunks[[i]]
  colnames(out) = as.character(roll$chunks$end.date)
  write.csv(out,
            file = file.path("guardian", "topWordsPerChunk", paste0(i, "_", topwords[i], ".csv")),
            fileEncoding = "UTF-8")
}
...
for(rel in c(0.8, 0.85)){
  ...
  saveRDS(quantiles1, file.path("guardian", paste0("quantiles", rel, ".rds")))
  saveRDS(sim1, file.path("guardian", paste0("sim", rel, ".rds")))
  ...
  pdf(file.path("guardian", paste0("wordimpact", rel, ".pdf")), height = 8, width = 10)
  for(k in seq_len(K)){
    zaehler = 0
  ...
  fwrite(events, file = file.path("guardian", paste0("changes", rel, ".csv")))
  
  pdf(file.path("guardian", paste0("changes", rel, ".pdf")), height = 8, width = 8)
